    ...
    <div class="content">
        <div style="margin:15px">
            ...
            <div style="height: 60px;  background-color: #FCFAF7;">
                <!-- 发布时间 -->
                <text style="position: absolute; left: 10px; top: 15px; color: #808080;">发布时间：{{ articleInfo.created_at }}
                </text>
                <!-- 分类信息 -->
...
const loadCategories = async () => {
    categoryOptions.value = config.menuItems.filter(item => item.mainMenu == '/consumption/detail').map((item) => {
        return {
            label: item.label,
            value: item.index
        }
...
const loadArticle = async () => {
    articleDetail('consumptionarticle', route.query.id).then(result => {
        if (result != null) {
